...
const sheetVariants = cva(
  // Apple 半高 Sheet：玻璃材质、圆角 28、左右/下方 inset 12pt，不贴屏幕边缘
  "glass-pop fixed z-50 gap-4 rounded-3xl transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
        right:
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-full p-1.5 text-faint opacity-70 transition-opacity hover:bg-fill hover:opacity-100 focus:outline-none">
        <X className="h-4 w-4" />
