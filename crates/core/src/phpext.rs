...
fn meta_for(name: &str) -> Option<Meta> {
    let (label, group, hint) = match name {
        "core" => ("Core", "基础", "PHP 核心，不可禁用"),
        "standard" => ("Standard", "基础", "标准库函数集"),
        "spl" => ("SPL", "基础", "标准 PHP 库（内置）"),
        "pcre" => ("PCRE", "基础", "正则表达式（内置）"),
        "date" => ("Date", "基础", "日期时间（内置）"),
        "json" => ("JSON", "基础", "JSON 编解码（内置）"),
        "hash" => ("Hash", "基础", "哈希算法（内置）"),
        "reflection" => ("Reflection", "基础", "反射（内置）"),
        "session" => ("Session", "基础", "会话管理"),
        "filter" => ("Filter", "基础", "数据过滤与校验"),
        "ctype" => ("Ctype", "基础", "字符类型检查"),
        "tokenizer" => ("Tokenizer", "基础", "词法分析（Composer 需要）"),
        "iconv" => ("Iconv", "基础", "字符集转换"),
        "zlib" => ("Zlib", "基础", "压缩流（内置）"),

        "openssl" => ("OpenSSL", "安全", "HTTPS / 加密 / 证书"),
        "sodium" => ("Sodium", "安全", "现代加密库"),
        "mcrypt" => ("Mcrypt", "安全", "旧式加密（已废弃，老项目才用）"),

        "curl" => ("cURL", "网络", "HTTP 客户端，调第三方接口必备"),
        "sockets" => ("Sockets", "网络", "底层 socket"),
        "ftp" => ("FTP", "网络", "FTP 客户端"),
        "imap" => ("IMAP", "网络", "收邮件"),
        "soap" => ("SOAP", "网络", "SOAP 客户端/服务端"),
        "ldap" => ("LDAP", "网络", "目录服务"),

        "pdo" => ("PDO", "数据库", "PDO 抽象层（其它 pdo_* 的前置）"),
        "pdo_mysql" => ("PDO MySQL", "数据库", "PDO 连 MySQL —— Laravel 等框架默认走这条"),
        "pdo_sqlite" => ("PDO SQLite", "数据库", "PDO 连 SQLite"),
        "pdo_pgsql" => ("PDO PostgreSQL", "数据库", "PDO 连 PostgreSQL"),
        "pdo_sqlsrv" => ("PDO SQL Server", "数据库", "PDO 连 SQL Server"),
        "pdo_oci" => ("PDO Oracle", "数据库", "PDO 连 Oracle"),
        "mysqli" => ("MySQLi", "数据库", "MySQL 原生扩展（WordPress 用它）"),
        "mysqlnd" => ("MySQLnd", "数据库", "MySQL 原生驱动（mysqli / pdo_mysql 的底层）"),
        "pgsql" => ("PostgreSQL", "数据库", "PostgreSQL 原生扩展"),
        "sqlite3" => ("SQLite3", "数据库", "SQLite 原生扩展"),
        "mongodb" => ("MongoDB", "数据库", "MongoDB 驱动（需另外下载）"),
        "oci8" => ("OCI8", "数据库", "Oracle 原生扩展"),
        "odbc" => ("ODBC", "数据库", "ODBC 数据源"),
        "sqlsrv" => ("SQL Server", "数据库", "SQL Server 原生扩展"),
        "redis" => ("Redis", "缓存", "Redis 客户端（连接本地 Redis / 队列）"),
        "memcached" => ("Memcached", "缓存", "Memcached 客户端"),
        "apcu" => ("APCu", "缓存", "用户态缓存（Laravel 可选用）"),
        "igbinary" => ("igbinary", "缓存", "更紧凑的序列化，Redis/Session 可选"),

        "gd" => ("GD", "图像", "图像处理（验证码 / 缩略图）"),
        "gd2" => ("GD2", "图像", "图像处理（PHP 7.4 及以前）"),
        "imagick" => ("ImageMagick", "图像", "ImageMagick 绑定（需另外下载）"),
        "exif" => ("EXIF", "图像", "读取图片元数据"),
        "gmagick" => ("GraphicsMagick", "图像", "GraphicsMagick 绑定"),

        "mbstring" => ("mbstring", "文本", "多字节字符串（中文项目几乎必开）"),
        "intl" => ("Intl", "文本", "国际化（ICU）—— 时间/货币/多语言格式化"),
        "gettext" => ("Gettext", "文本", "gettext 多语言"),
        "pspell" => ("Pspell", "文本", "拼写检查"),
        "enchant" => ("Enchant", "文本", "拼写检查（Enchant 后端）"),

        "zip" => ("Zip", "归档", "zip 读写（Composer 装包要用）"),
        "bz2" => ("Bzip2", "归档", "bzip2 压缩"),
        "phar" => ("Phar", "归档", "PHP 归档（Composer 依赖）"),
        "zend opcache" => ("OPcache", "性能", "字节码缓存 —— 生产环境必开"),
        "opcache" => ("OPcache", "性能", "字节码缓存 —— 生产环境必开"),
        "xdebug" => ("Xdebug", "调试", "断点调试 / 性能剖析（配合 IDE）"),
        "uploadprogress" => ("Upload Progress", "调试", "上传进度回调（老框架用）"),

        "fileinfo" => ("Fileinfo", "文件", "识别文件真实 MIME 类型"),
        "exif_alt" => ("EXIF", "文件", "图片元数据"),
        "ffi" => ("FFI", "文件", "调用 C 库"),
        "com_dotnet" => ("COM / .NET", "系统", "调用 Windows COM 组件"),
        "sysvshm" => ("SysV SHM", "系统", "共享内存"),
        "sysvsem" => ("SysV Sem", "系统", "信号量"),
        "shmop" => ("Shmop", "系统", "共享内存操作"),
        "pcntl" => ("PCNTL", "系统", "进程控制（CLI 用；Windows 不支持）"),
        "posix" => ("POSIX", "系统", "POSIX 接口（Windows 不支持）"),
        "readline" => ("Readline", "系统", "交互式命令行"),
        "snmp" => ("SNMP", "系统", "网络设备监控"),
        "yaml" => ("YAML", "系统", "YAML 解析（需另外下载）"),
        "dba" => ("DBA", "系统", "dbm 风格数据库"),
        "interbase" => ("InterBase", "系统", "Firebird / InterBase"),
        "tidy" => ("Tidy", "系统", "HTML 清理与修复"),
        "xmlrpc" => ("XML-RPC", "系统", "XML-RPC（已废弃）"),
        _ => ("", "", ""),
    };
    if label.is_empty() {
        None
