...
fn main() {
    ...
    for key in install {
        check!(
            format!("安装 {key}"),
            rt.block_on(state.install_package(key))
    ...
    for key in ["memcached", "mailpit"] {
        check!(
            format!("启动 {key}"),
            state.start_service(key).map(|_| "健康检查通过".to_string())
        );
    }
    // Caddy 需要 Caddyfile：验证「无配置时按模板生成再启动」
    check!(
        "启动 caddy（自动生成 Caddyfile）",
        state
            .start_service("caddy")
            .map(|_| "配置模板已生成并启动".to_string())
    );
    ...
    check!(
        ...
        (|| -> Result<String, String> {
            ...
            if m >= 20000 && ml >= 20000 {
                Ok(format!(
                    "memcached={m} mailpit={ml}（默认 11211/8025 已避让）"
                ))
            } else {
    ...
    for key in ["caddy", "mailpit", "memcached"] {
        check!(
            format!("停止 {key}"),
            state.stop_service(key).map(|_| "已停止".to_string())
        );
    }
    ...
    check!(
        ...
        (|| -> Result<String, String> {
            ...
            if leaked.is_empty() {
                Ok("全部端口已释放".to_string())
            } else {
                Err(format!("仍在运行: {leaked:?}"))
            }
        })()
    );
