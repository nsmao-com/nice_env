...
fn main() {
    ...
    let node = run_cmd(
        &rt.join("node/22.14.0/node-v22.14.0-win-x64/node.exe"),
        &["--version"],
    );
    check!("Node 版本", node);
    ...
    check!("启动 PHP 8.5.10 池", {
        state
            .start_service("php@8.5.10")
            .map(|_| "php-cgi ×4 已监听".to_string())
    });
    check!("启动 Apache 2.4.66", {
        state
            .start_service("apache")
            .map(|_| "8180 端口就绪".to_string())
    });
    ...
    /* ---------- 3.5 多版本能力 ---------- */
    check!(
        "nginx 切换使用版本 1.28 → 启动",
        (|| -> Result<String, nsb_core::AppError> {
    ...
    check!(
        "Node 20/18",
        run_cmd(
            &rt2.join("node/20.19.5/node-v20.19.5-win-x64/node.exe"),
            &["--version"]
        )
        .and_then(|a| run_cmd(
            &rt2.join("node/18.20.8/node-v18.20.8-win-x64/node.exe"),
            &["--version"]
        )
        .map(|b| format!("{a} / {b}")))
    );
    check!(
        "Python 3.13/3.11",
        run_cmd(&rt2.join("python/3.13.7/python.exe"), &["--version"]).and_then(|a| run_cmd(
            &rt2.join("python/3.11.9/python.exe"),
            &["--version"]
        )
        .map(|b| format!("{a} / {b}")))
    );
    check!(
        "Go 1.23",
        run_cmd(&rt2.join("go/1.23.6/go/bin/go.exe"), &["version"])
    );
    ...
    let nsb_home = std::env::current_dir()
        .unwrap_or_default()
        .join(".extra-home");
    ...
    check!("无孤儿进程", {
        if leftovers.is_empty() && orphans == 0 {
            ...
            Err(format!(
                "仍占用端口: {leftovers:?}；残留 NSB 进程数: {orphans}"
            ))
        }
    });
